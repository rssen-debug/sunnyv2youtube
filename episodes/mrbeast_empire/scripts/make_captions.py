#!/usr/bin/env python3
"""CAPTIONS v2 — kinetic typography (sunnyv2/Shorts-pop) — EPISODE-EDITION (mrbeast_empire)

Samma motor som scripts/make_captions.py i repo-roten, plus:
  * TRAIL: block kan ha en tyst svans (klippfönster). timings.json får då både
    "dur" (hela blockfönstret, inkl. svans) och "speech" (talad del). Orden
    fördelas bara över "speech" => captions håller exakt takt mot VO medan
    klippet spelar i svansen.
  * keywords.txt i episodroten styr röda keyword-highlight.

Kör som script -> timings.json + captions.ass
"""
import os, json, wave
...
# Sekunder tyst svans per block (klipp spelas där). Sätter captions endast på tal-delen.
TRAIL = {"A3": 7.6, "A4": 14.2, "B2": 7.2}
...
def build_events(segments, keywords, lead=LEAD, jitter=True):
    """segments: [{id,text,start,dur,speech}] -> [(start, end, ass_text)]"""
    ev = []
    for s in segments:
        span_len = s.get("speech", s["dur"])
        words = s["text"].split()
        weights = [len(w) + 2 for w in words]
        t0 = s["start"] + lead
        span = max(span_len - lead - 0.15, 0.4)
        wt, acc = [], t0
...
def main():
    ...
    for b in blocks:
        ...
        d = wav_dur(p)
        sp = max(d - TRAIL.get(b["id"], 0.0), 0.5)
        segs.append({"id": b["id"], "text": b["text"], "start": round(t, 3),
                     "dur": round(d, 3), "speech": round(sp, 3)})
        t += d + GAP
    total = t - GAP + TAIL
    ...
    for s in segs:
        print(f"  {s['id']:>3}  start {s['start']:7.2f}  dur {s['dur']:6.2f}  speech {s['speech']:6.2f}")
    ...
    print("captions.ass:", len(ev), "events (v2 kinetic, trail-aware)")
